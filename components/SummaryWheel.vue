  ...
  <div class="chart-summary">
    <section>
      <div class="sismograf-report">
        <!-- <pre>{{ globalLevel }}</pre> -->
        <!-- <pre>{{ domainsData }}</pre> -->
        <!-- <pre>{{ quartersData }}</pre> -->
        <!-- <pre>{{ principlesValuesData }}</pre> -->
        <b-row>
          <b-col md="2">
            <div class="analysis-detail" v-if="title">
              <h2>{{ title }}</h2>
            </div>
          ...
          <b-col md="7">
            <div :id="elementId" class="svg svg-result"></div>
          </b-col>
...
export default {
  ...
  computed: {
    ...
    quartersData() {
      const quarters = [];
      this.chartSummary.domains.forEach((d, i) => {
        quarters.push({ name: `q${i}v`, value: this.toPct(d.resilienceLevel) });
        quarters.push({ name: `q${i}`, value: this.toPct(d.resilienceLevel) });
      });
    ...
    domainsValuesData() {
      ...
      this.chartSummary.domains.forEach((d, i) => {
        domainsValues.push({
          name: `q${i}v`,
          value: this.toPct(d.resilienceLevel),
        });
        domainsValues.push({
          name: `q${i}`,
          value: 100 - this.toPct(d.resilienceLevel),
        });
      });
      ...
      this.chartSummary.domains.forEach((d, i) => {
        d.principles.forEach((p, j) => {
          principles.push({
            ...
            value: this.toPct(p.resilienceLevel),
          });
          principles.push({
            label: `p${3 * i + j}`,
            value: 100 - this.toPct(p.resilienceLevel),
          });
          c++;
        });
      });
      for(let i = principles.length / 2; i < 12; i++){
        principles.push({ label: `p${i}v`, description: '', value: null, hidden: true})
    ...
    globalLevel() {
      return this.toPct(this.chartSummary.resilienceLevel)
    }
  },
  mounted() {
    this.draw();      
    const svg = document.getElementById(this.elementId)
    const ratio = window.innerWidth / 1920 * (this.compare ? 0.8 : 1)
    if (svg) {
    ...
    async draw() {
      ...
      const domains = () => {
        ...
        return labels.map((label, i) => {
          return { label: label, value: 90, i, hidden: this.domainsData[i].hidden, };
        });
      };
      ...
      pct
        ...
        // .attr("dy", "-2.5em")
        .text(function (d) {
          return domainsValues()[d.data.i * 2].value ? `${domainsValues()[d.data.i * 2].value}%` : '';
        })
        .style("font-family", "Athletics")
      ...
      textpv
        ...
        // .attr("dy", "-2.5em")
        .text(function (d) {
          return principlesValues()[d.data.i * 2].value ? `${principlesValues()[d.data.i * 2].value}%` : '';
        })
        .style("font-family", "Athletics")
        .style("font-size", "22px")
        .style("font-weight", "500");
